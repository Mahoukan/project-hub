...
function createCheckerSpinPattern () {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      grid[row][col] =
        row % 2 === 0 ? col % SPRITE_ROTATIONS : (col + 2) % SPRITE_ROTATIONS
    }
  }
...
function createRadialSpokesPattern () {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      grid[row][col] =
        floor(normalisedAngle * SPRITE_ROTATIONS) % SPRITE_ROTATIONS
    }
  }
...
function getRecoloredImage (baseImg, lightCol, darkCol) {
  let key =
    baseImg.recolorId + '-' + lightCol.toString() + '-' + darkCol.toString()
...
function drawHelpOverlay () {
  ...
  fill(255, alpha * 0.7)
  rect(width / 2 - RESET_ZONE_WIDTH / 2, 0, RESET_ZONE_WIDTH, RESET_ZONE_HEIGHT)
...
function drawSpriteMenu () {
  ...
  text(
    menuOpen ? '×' : '☰',
    MENU_X + MENU_BUTTON_SIZE / 2,
    MENU_Y + MENU_BUTTON_SIZE / 2
  )
  ...
  if (menuOpen) {
    ...
    text(
      'Dark',
      colourLabelX,
      colourY + MENU_ITEM_HEIGHT + MENU_GAP + MENU_ITEM_HEIGHT / 2
    )
  } else {
    hideColourPickers()
  }
